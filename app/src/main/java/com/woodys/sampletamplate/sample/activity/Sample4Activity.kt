 ...
 */
class Sample4Activity : ToolBarActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val flowBaseAdapter = object : FlowBaseAdapter<String>(mData.toList()) {
            override fun getView(context: Context, parent: ViewGroup, position: Int): View {
                return LayoutInflater.from(context).inflate(R.layout.item_4_text_view, null)
            }

            override fun bindView(view: View, position: Int) {
                val item = getItem(position)
                val textView = view.findViewById<TextView>(R.id.white_item_selector)
                textView.setBackgroundColor(colorItems[Random().nextInt(colorItems.size)])
                textView.text = item
            }
        }
